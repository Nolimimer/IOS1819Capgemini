...
class ScanBoundingBox: SCNNode {
    ...
    var extent = float3(0.1, 0.1, 0.1) {
        didSet {
            extent = max(extent, minSize)
            updateVisualization()
            NotificationCenter.default.post(name: ScanBoundingBox.extentChangedNotification,
                                            object: self)
        }
    }
    ...
    override var simdPosition: float3 {
        willSet(newValue) {
            if distance(newValue, simdPosition) > 0.001 {
                NotificationCenter.default.post(name: ScanBoundingBox.positionChangedNotification,
                                                object: self)
            }
        }
    }
    ...
    func updateCapturingProgress() {
        ...
        if self.progressPercentage != progressPercentage {
            ...
            NotificationCenter.default.post(name: ScanBoundingBox.scanPercentageChangedNotification,
                                            object: self,
                                            userInfo: [ScanBoundingBox.scanPercentageUserInfoKey: progressPercentage])
        }
