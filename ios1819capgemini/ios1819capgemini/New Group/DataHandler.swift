...
//             removeIncidentNode(incident: incident)
             incidents.remove(at: index)
             saveToJSON()
             return
        }
    }
    static func removeIncidentNode(incident: Incident) {
        for (index, node) in nodes.enumerated() where node.name == "\(incident.identifier)" {
